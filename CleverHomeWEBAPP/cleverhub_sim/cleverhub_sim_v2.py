...
class HouseState(object):
    ...
    def update_simulation(self):
        ...
        if self.__humidifier_state: 
            self.__humidity += 1
        else:
            self.__humidity -= 1
        if self.__humidity < 0: self.humidity = 0
        if self.__humidity > 100: self.humidity = 100
    ...
    def set_state(self, new_state):
        ...
        for par in params:
            ...
            if k[:2] == "LS":
                idx_light = int(k[2])
                if v == "1": self.set_light(idx_light, True)
                else: self.set_light(idx_light, False)
            elif k[:2] == "DS":
                idx_door = int(k[2])
                if v == "1": self.set_door(idx_door, True)
                else: self.set_door(idx_door, False)
            elif k[:2] == "CO":
                idx_camera = int(k[2])
                if v == "1": self.set_camera(idx_camera, True)
                else: self.set_camera(idx_camera, False)
            elif k == "AS":
                if v == "1": self.set_alarm_state(True)
                else: 
                    self.set_alarm_state(False)
                    self.set_alarm_sounding(False)
            elif k == "AO":
                if v == "1": self.set_alarm_sounding(True)
            elif k == "HS":
                if v == "1": self.set_heater_state(True)
                else: self.set_heater_state(False)
            elif k == "CS":
                if v == "1": self.set_chiller_state(True)
                else: self.set_chiller_state(False)
            elif k == "HUS":
                if v == "1": self.set_humidifier_state(True)
                else: self.set_humidifier_state(False)

    # Getters and setters for house properties
    def get_temperature(self): return self.__temperature

    def set_door(self, idx, value): self.__doors[idx] = value
    def get_door(self, idx):
        if self.__doors[idx]: return "1"
        return "0"

    def set_light(self, idx, value): self.__lights[idx] = value
    def get_light(self, idx):
        if self.__lights[idx]: return "1"
        return "0"

    def set_proximity(self, idx, value): self.__proximity[idx] = value
    def get_proximity(self, idx):
        if self.__proximity[idx]: return "1"
        return "0"

    def set_camera(self, idx, value): self.__cameras[idx] = value
    def get_camera(self, idx):
        if self.__cameras[idx]: return "1"
        return "0"
        
    def set_camera_motion(self, idx, value): self.__cameras_motion[idx] = value
    def get_camera_motion(self, idx):
        if self.__cameras_motion[idx]: return "1"
        return "0"        
    ...
    def set_alarm_sounding(self, a): self.__alarm_sounding = a
    def get_alarm_sounding(self):
        if self.__alarm_sounding: return "1"
        return "0"
    ...
    def set_humidifier_state(self, h): self.__humidifier_state = h
    def get_humidifier_state(self):
        if self.__humidifier_state: return "1"
        return "0"        
    ...
    def get_home(self): return self.__home
    def get_target_temperature(self): return self.__target_temp
    ...
    def get_state(self):
        ...
        for door_idx in range(0, len(self.__doors)):
            state = state + "DS{0}={1};".format(door_idx, self.get_door(door_idx))

        for light_idx in range(0, len(self.__lights)):
            state = state + "LS{0}={1};".format(light_idx, self.get_light(light_idx))

        for prox_idx in range(0, len(self.__proximity)):
            state = state + "PS{0}={1};".format(prox_idx, self.get_proximity(prox_idx))
      
        for cam_idx in range(0, len(self.__cameras)):
            state = state + "CO{0}={1};".format(cam_idx, self.get_camera(cam_idx))
            
        for cam_mot_idx in range(0, len(self.__cameras_motion)):
            state = state + "CM{0}={1};".format(cam_mot_idx, self.get_camera_motion(cam_mot_idx))            
            
        state = state + f"TR={self.get_temperature()};" \
                        f"AS={self.get_alarm_state()};" \
                        f"AO={self.get_alarm_sounding()};" \
                        f"HS={self.get_heater_state()};" \
                        f"CS={self.get_chiller_state()};" \
                        f"HUS={self.get_humidifier_state()};" \
                        f"HUR={self.get_humidity()}"
        return state
    ...
    def run(self):
        ...
        try:
            while not self.__stop.is_set():
                ...
                cmd = input("\nEnter a command: d=[toggle door], l=[toggle light], p=[toggle proximity], c=[toggle camera], m=[toggle camera motion], RET=[show current status]: ")

                if cmd == "d":
                    idx = int(input("Enter the door number: "))
                    if self.house.get_door(idx) == "1": self.house.set_door(idx, False)
                    else: self.house.set_door(idx, True)
                    print ("door is now {}".format(self.house.get_door(idx)))
                elif cmd == "l":
                    idx = int(input("Enter the light number: "))
                    if self.house.get_light(idx) == "1": self.house.set_light(idx, False)
                    else: self.house.set_light(idx,True)
                    print ("light is now {}".format(self.house.get_light(idx)))
                elif cmd == "p":
                    idx = int(input("Enter the proximity sensor number: "))
                    if self.house.get_proximity(idx) == "1": self.house.set_proximity(idx, False)
                    else: self.house.set_proximity(idx, True)
                    print ("proximity is now {}".format(self.house.get_proximity(idx)))
                elif cmd == "c":
                    idx = int(input("Enter the camera number: "))
                    if self.house.get_camera(idx) == "1": self.house.set_camera(idx, False)
                    else: self.house.set_camera(idx, True)
                    print ("camera is now {}".format(self.house.get_camera(idx)))
                elif cmd == "m":
                    idx = int(input("Enter the camera number: "))
                    if self.house.get_camera_motion(idx) == "1": self.house.set_camera_motion(idx, False)
                    else: self.house.set_camera_motion(idx, True)
                    print ("camera motion is now {}".format(self.house.get_camera_motion(idx)))                           
    ...
    def handle_next_request(self):
        ...
        if data:
            if data[:3] == "ACC":
                ...
                raise Exception("Connection was refused; check if user/password are correct, or if there is already a Hub with the same HOM name connected.")
            elif data[:2] == "GS":
...
def main():
    ...
    print ('Will connect to {0}:{1}'.format(server_address[0], server_address[1]), flush=True)
    print ('Will use user and pass {0}:{1}'.format(username, password), flush=True)
